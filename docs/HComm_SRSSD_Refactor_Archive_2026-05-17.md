...
**文档导航**
- 文档索引页：[`docs/HComm_Doc_Index.md`](./HComm_Doc_Index.md)
- HComm 集成说明：[`tracy_hcom/README.md`](../tracy_hcom/README.md)
...
### 9.1 当前遗留项（phase-one 已知）
- `TracyHcom.cpp` 仍通过 include 汇聚 `public/client` 与 `public/common` 的实现文件。
- Lite 主路径仍存在 `TracyProfiler.hpp` 时间/线程相关残余依赖。
- `TracyHcomStubs.cpp` 仍承担 phase-one 过渡符号补齐角色。
- `TRACYHCOM_NO_TRACY_PROFILER` 目前是开关预留，尚未形成完整无-Profiler主路径。

### 9.2 phase-two 推荐落地顺序
1. 时间源替换：先将 Lite 路径时间读取切到 Hcom shim，并保证导出时间轴换算正确。
2. 线程符号替换：收敛线程句柄/线程名链表到 Hcom 自有实现。
3. 删除过渡层：移除 `TracyHcomStubs.cpp`，收口 no-Profiler 开关到可用主路径。

### 9.3 验收标准（phase-two 完成判据）
- 打开 no-Profiler 路径时，编译不再依赖 `TracyProfiler.hpp`。
- 不再通过 stubs 补时间/线程关键符号。
- Core-only 与 Perfetto 两种模式均可编译通过。
- 导出 trace 的时间线连续、线程归属正确。
...
- 已知风险：
  - phase-one 仍存在对 `TracyProfiler.hpp` 的残余依赖（通过 stub 过渡）。
	- 外部仓若追求"极少文件直接编译"，当前阶段仍需最小子集 bundle（非单文件级）。
- 后续计划：
