...
## 10. Windows 基线补记（2026-05-24，MSVC x64 / Release）

环境：Windows / MSVC x64 Dev Shell / `win-all-hcomm-release` 预设，
目标：`TracyNoSendPerfRunner`，每轮执行 `zone` + `export`，共 5 轮。

原始输出：

| 轮次 | zone elapsed_ns | zone ns_per_pair | export total_us | serialize_us | write_us | file_size(bytes) | write_throughput_mb_s |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 4,133,500 | 20.67 | 100,495 | 93,732 | 6,763 | 16,398,560 | 2312.42 |
| 2 | 3,415,700 | 17.08 | 110,747 | 102,595 | 8,152 | 16,218,561 | 1897.35 |
| 3 | 4,194,800 | 20.97 | 109,739 | 100,773 | 8,966 | 16,398,565 | 1744.24 |
| 4 | 4,060,400 | 20.30 | 113,124 | 103,180 | 9,944 | 16,398,560 | 1572.70 |
| 5 | 4,211,100 | 21.06 | 118,766 | 106,708 | 12,058 | 16,398,565 | 1296.97 |

5 轮均值（作为当前 Windows baseline）：

- `zone elapsed_ns`: **4,003,100 ns**
- `zone ns_per_pair`: **20.02 ns**
- `export total_elapsed_us`: **110,574 us**
- `serialize_us`: **101,398 us**
- `write_us`: **9,177 us**
- `file_size`: **16,362,562 bytes**（约 15.60 MiB）
- `write_throughput_mb_s`: **1764.74 MB/s**

说明：

- `zone ns_per_pair` 已稳定在 ~20ns 档位，和此前 Release 基线同量级。
- `serialize_us` 仍是导出主耗时，`write_us` 受文件系统缓存抖动影响较大。
- 该基线可用于后续与 PGO、Linux/WSL2 或 phase-two 路径对比。
